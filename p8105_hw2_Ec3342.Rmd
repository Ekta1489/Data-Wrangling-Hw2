...
*The key variables in the combined_data would be month, year, total. The key variables in the trashwheel_data would be dumpster, month, year.
...
##Loading and tidying the data Popular_Baby_Names and creating a dataset called pop_baby. Changing the case structure of gender, ethnicity and childs first name to lower case to maintain uniformity throughout. Removing all the duplicate rows.
...
pop_baby = read_csv(file = "./data/Popular_Baby_Names.csv") %>%
  ...
  distinct()
```
##Producing a well-structured, reader-friendly table showing the rank in popularity of the name “Olivia” as a female baby name over time; showing the rows for ethnicities and columns for year. 

```{r olivia}

olivia = filter(pop_baby, gender == "female", childs_first_name == "olivia") %>%
  select(-gender, -childs_first_name, -count)
  olivia_new = pivot_wider(
    olivia, names_from = "year_of_birth", values_from = "rank") 
  olivia_new[is.na(olivia_new)] <- 0
kable(olivia_new, format = "html")
```
